...
export default function EditContent({ content, contentIndex, onContentUpdated, onCancel }: EditContentProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/contents', {
        ...
        body: JSON.stringify({ index: contentIndex, data: formData })
      });
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-600">Update publication details</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-slate-200 p-8">
        ...
        <div className="flex gap-3">
          ...
          <button
            type="button"
            onClick={onCancel}
            className="flex items-center gap-2 px-6 py-2.5 bg-slate-200 text-slate-900 font-medium rounded-lg hover:bg-slate-300 transition-colors"
          >
            Cancel
          </button>
        </div>
